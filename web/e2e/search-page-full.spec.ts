...
test.describe('Search Input (Real API)', () => {
  ...
  test('TC_SEARCH_INPUT_001: searches with a single keyword and shows results', async ({ page }) => {
    await page.goto(searchUrl(searchSeedKeyword()))
...
authedTest.describe('Search Sort and Filter (Authenticated Real API)', () => {
  authedTest.beforeEach(async ({ page }) => {
    await setEnglishLocale(page)
  })

  // TC_SEARCH_SORT_001 P0 - default relevance tab selected
  authedTest('TC_SEARCH_SORT_001: relevance sort tab is selected by default', async ({ page }) => {
    await page.goto(searchUrl(searchSeedKeyword(), 'relevance'))
    await expect(page.getByRole('button', { name: 'Relevance' })).toBeVisible()
  })

  // TC_SEARCH_SORT_004 P0 - downloads sort
  authedTest('TC_SEARCH_SORT_004: clicking Downloads tab updates sort in URL', async ({ page }) => {
    await page.goto(searchUrl(searchSeedKeyword(), 'relevance'))
  ...
  authedTest('TC_SEARCH_SORT_005: clicking Newest tab updates sort in URL', async ({ page }) => {
    await page.goto(searchUrl(searchSeedKeyword(), 'relevance'))
  ...
  authedTest('TC_SEARCH_SORT_006: switching sort tab preserves the search keyword', async ({ page }) => {
    await page.goto(searchUrl(searchSeedKeyword(), 'relevance'))
    await page.getByRole('button', { name: 'Downloads' }).click()
    await expect(page).toHaveURL(new RegExp(`q=${searchSeedKeyword()}`))
    await expect(page).toHaveURL(/sort=downloads/)
  ...
  authedTest('TC_SEARCH_SORT_007: switching sort tab resets page to 0', async ({ page }) => {
    await page.goto(searchUrl(searchSeedKeyword(), 'relevance', 1))
    await page.getByRole('button', { name: 'Downloads' }).click()
    await expect(page).toHaveURL(/page=0/)
  })

  // TC_SEARCH_SORT_012 P1 - URL contains sort param
  authedTest('TC_SEARCH_SORT_012: URL contains sort parameter after switching tabs', async ({ page }) => {
    await page.goto(searchUrl(searchSeedKeyword(), 'relevance'))
    await page.getByRole('button', { name: 'Downloads' }).click()
    await expect(page).toHaveURL(/sort=/)
  })

  authedTest('starred only filter stays on search page for authenticated user', async ({ page }) => {
    await page.goto(searchUrl(searchSeedKeyword(), 'relevance'))
...
test.describe('Search Results (Real API)', () => {
  ...
  test('TC_SEARCH_RESULT_009: results are sorted by downloads when Downloads tab is selected', async ({ page }) => {
    await page.goto(searchUrl(searchSeedKeyword(), 'downloads'))
    await expect(page).toHaveURL(/sort=downloads/)
  ...
  test('TC_SEARCH_RESULT_010: results are sorted by newest when Newest tab is selected', async ({ page }) => {
    await page.goto(searchUrl(searchSeedKeyword(), 'newest'))
    await expect(page.locator('body')).not.toContainText(/error|500/i)
  })
})
...
test.describe('Search Pagination (Real API)', () => {
  ...
  test('TC_SEARCH_PAGE_011: URL contains page parameter', async ({ page }) => {
    await page.goto(searchUrl(searchSeedKeyword(), 'relevance', 0))
  ...
  test('TC_SEARCH_PAGE_012: switching page preserves search keyword and sort', async ({ page }) => {
    await page.goto(searchUrl(searchSeedKeyword(), 'downloads', 0))
...
test.describe('Search Security (Real API)', () => {
  ...
  authedTest('TC_SEARCH_SEC_003: tampered URL parameters are handled gracefully', async ({ page }) => {
