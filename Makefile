...
agent-verify:
	@if [ -z "$(TASK)" ]; then \
		echo "Usage: make agent-verify TASK=<task-slug> [PARALLEL_WORKTREE_ROOT=/path] [SOURCES=\"branch1 branch2\"]"; \
		exit 1; \
	fi
	PARALLEL_WORKTREE_ROOT="$(PARALLEL_WORKTREE_ROOT)" ./scripts/agent-verify.sh "$(TASK)" $(SOURCES)
