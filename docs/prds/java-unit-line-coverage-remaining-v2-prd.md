...
- 任何单元测试、`DataJpaTest` 切片测试、以及用于 JaCoCo 行覆盖率门禁的测试，只要涉及 MySQL 持久化路径，都只能使用 `H2` 或 mock
- 这类测试不得真实连接 MySQL，也不得依赖 `MySQLContainer`
- 真实 MySQL 只允许保留给显式的 `IntegrationTest`、`RuntimeIntegrationTest`、迁移验证与端到端回归验证
- 当前已存在的真实 MySQL 单测，必须在本轮优先处理：迁为 `H2`/mock，或明确重分类为集成/运行时验证并移出单测补覆盖范围
...
- 参与单测覆盖率门禁的测试不再依赖真实 MySQL 或 `MySQLContainer`
