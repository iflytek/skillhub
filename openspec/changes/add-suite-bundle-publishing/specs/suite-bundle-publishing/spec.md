...
阻塞状态，并能通过操作 ID 在刷新或重新登录后恢复。“我的套件” SHALL 将技能套件与发布任务分为
两个页签，并在发布任务中按“需要处理”“进行中”“最近完成”分层展示当前用户发起的 Bundle 操作，
使尚未生成 Suite 草稿或已经取消的发布过程都有稳定入口。点击任务 SHALL 进入独立发布详情页，不得
返回创建或新版本编辑页面。公开套件专区 SHALL 继续只展示已发布 Suite，不得把执行操作伪装成已经
创建的 Suite。任务列表 SHALL 分页并在数据库中聚合成员进度；只有仍可能自动变化的运行或等待状态
才进行短间隔轮询。任务列表和详情首屏 SHALL 使用文本明确展示整体状态及与该状态对应的下一步，
...
- **THEN** “我的套件”的“发布任务”页签按状态分层展示目标坐标、目标版本、整体状态和成员完成数量
...
- **THEN** 操作以 `CANCELLED` 保留在“最近完成”分组，详情明确说明不会再创建 Suite 草稿
