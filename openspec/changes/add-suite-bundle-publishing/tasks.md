...
- [x] 3.6 `[REQ-SBP-17, REQ-SBP-18]` 在“创建 Suite”和“创建新版本”入口提供手工组合/本地导入选择，并实现准确列出副作用、最终可见性、发布路径和逐成员 warning 确认的确认页及可恢复进度页；展示包/引用状态、Skill 审核链接、阻塞原因、重试/取消和生成的 Suite 草稿；“我的套件”通过分页和数据库聚合展示当前用户发起的非终态操作并提供继续查看入口，只对运行或等待状态短间隔轮询。通过正常、等待、可重试阻塞、必须重预览、取消、权限变化、刷新、重新登录、超过单页容量和离开导入页后恢复用例验证。
