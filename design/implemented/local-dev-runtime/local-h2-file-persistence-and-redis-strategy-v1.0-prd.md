...
> 状态更新（2026-05-04）：本文形成于 PostgreSQL 仍是标准本地运行时的阶段。当前仓库默认/标准运行时已切换为 `MySQL 8 + Redis + local-file-index`，`local-h2` 继续作为轻量联调与测试模式保留。下文对 PostgreSQL 标准模式的引用仅代表当时背景，不再代表当前主路径。
